...
**Quarter synthesis:** post-Investor Day (Jun-24), the debate shifted to "is the $5B FY27 DC guide credible and at what multiple should QCOM trade?" — MS upgraded UW→EW ("We have been wrong to be skeptical"), Citi raised PT and is closer to Neutral, BofA/Barclays stay cautious, JPM Neutral at $265. From 06-26: TSMC confirmed as the Dragonfly C100/AI300 foundry (3nm/2nm + CoWoS/SoIC). From today's compiled briefing highlighting the Investor Day outputs: (1) **META multi-gen CPU deal** + MSFT HBC endorsement = two hyperscalers publicly committed, de-risking the $5B FY27 guide; (2) **Modular AI acquisition ($4B)** addresses the primary software-stack gap that is the #1 bear objection to QCOM vs NVDA CUDA in the data center. Net: supply-chain (TSMC) + software (Modular) + customer proof (META/MSFT) are all more positive than investors expected; the remaining debate is execution timing and the FY29E $40B non-handset target credibility.
